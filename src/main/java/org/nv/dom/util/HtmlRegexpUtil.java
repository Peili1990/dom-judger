...
public class HtmlRegexpUtil {
	 ...
	 * @return String
	 */
	public static String recoverTag(String input) {
		StringBuffer filtered = new StringBuffer(input.length());
		char c;
		for (int i = 0; i <= input.length() - 1; i++) {
			c = input.charAt(i);
			if(c == '&'){
				if(i+4 <= input.length() && input.substring(i, i+4).equals("&lt;")){
					filtered.append('<');
					i=i+3;					
				} else if(i+4 <= input.length() && input.substring(i, i+4).equals("&gt;")) {
					filtered.append('>');
					i=i+3;
				} else if(i+5 <= input.length() && input.substring(i, i+5).equals("&amp;")){
					filtered.append('&');
					i=i+4;
				} else if(i+6 <= input.length() && input.substring(i, i+6).equals("&quot;")){
					filtered.append('"');
					i=i+5;
				} else {
					filtered.append(c);
