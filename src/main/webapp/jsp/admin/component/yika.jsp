...
function yikaList(){
	...
	if(point <= 6){
		yika = "晚上仿佛听到"+rollyikaList(groupGP.concat(groupGS).concat(groupBK).concat(groupBS),2,groupNM,2)+"说话的声音。";	
	}else if(point > 6 && point <= 10){
		yika = rollyikaList(groupGP.concat(groupGS).concat(groupNM),2,groupBK.concat(groupBS),2)+"的样子不像是好人。";
	}else if(point > 10 && point <= 14){
		yika = rollyikaList(groupGP.concat(groupGS),2,groupBK.concat(groupBS).concat(groupNM),2)+"的样子像是好人。";
	}else if(point > 14 && point <= 16){
		yika = rollyikaList(groupGP.concat(groupGS).concat(groupBS).concat(groupNM),3,groupBK,1)+"的样子像是杀手。";
	}else if(point > 16 && point <= 18){
		yika = rollyikaList(groupGP,1,groupBK.concat(groupGS).concat(groupBS).concat(groupNM),3)+"的样子像是警察。";
	}else if(point == 19){
...
function rollyikaList(groupOne,numOne,groupTwo,numTwo){
	var result = [];
	groupOne.shuffle();
	groupTwo.shuffle();
